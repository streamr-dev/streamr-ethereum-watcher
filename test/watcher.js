...
describe("Watcher", () => {
    ...
    describe("real-time watcher", () => {
        ...
        it("catches product creation", async () => {
            ...
            await sendFrom(accounts[0], marketplace.methods.createProduct(productIdHex, "Event tester", accounts[3], 1e18, Currency.DATA, 1))
            ...
            assert.equal(cb.args[0][1].pricePerSecond, 1e9)
        ...
        it("catches product info update", async () => {
            ...
            await sendFrom(accounts[0], marketplace.methods.updateProduct(productIdHex, "Muh produx111!", accounts[4], 2e18, Currency.DATA, 10))
            assert.equal(cb.callCount, 1)
            assert.equal(cb.args[0][0], productIdBytes)
            assert.equal(cb.args[0][1].ownerAddress, accounts[0])
            assert.equal(cb.args[0][1].beneficiaryAddress, accounts[4])
            assert.equal(cb.args[0][1].minimumSubscriptionInSeconds, 10)
            assert.equal(cb.args[0][1].pricePerSecond, 2e9)
            assert.equal(cb.args[0][1].priceCurrency, "DATA")
        ...
        it("catches subscription", async () => {
            ...
            const verbose = true
            await sendFrom(accounts[0], token.methods.mint(accounts[1], BN(1000e18)))
            await sendFrom(accounts[1], token.methods.approve(marketplace.options.address, BN(200e18)))
    ...
    describe("playback", () => {
        it("catches product creation/deletion/redeploy/update/buy", async () => {
            const cb = sinon.spy()
            watcher.on("productDeployed", cb)
            await sendFrom(accounts[0], marketplace.methods.createProduct("0x1234", "playback-test", accounts[0], BN(1e18), Currency.DATA, 1)) // -> productDeployed
            ...
            await sendFrom(accounts[0], token.methods.mint(accounts[1], BN(1000e18)))
            await sendFrom(accounts[1], token.methods.approve(marketplace.options.address, BN(1000e18)))
            ...
            await sendFrom(accounts[0], marketplace.methods.updateProduct("0x1234", "updated-name", accounts[3], BN(2e18), Currency.DATA, 10)) // -> productUpdated
            await sendFrom(accounts[0], marketplace.methods.createProduct("0x2345", "playback-test-end", accounts[0], BN(1e18), Currency.DATA, 1)) // -> productDeployed
