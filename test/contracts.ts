...
describe("Contracts", () => {
    it("create product + subscribe", async function () {
        ...
        const tx1 = await marketplace.createProduct(productIdBytes32, "Contract tester", wallet.address, 1, MarketplaceEnums.Currency.DATA, 1)
        ...
        assert.deepStrictEqual(tr2.events.map((e: ethers.Event) => e.event), ["Mint", "Transfer"])
        const tx3 = await token2.approve(marketplace.address, 10000)
        const tr3 = await tx3.wait(0)
        assert.deepStrictEqual(tr3.events.map((e: ethers.Event) => e.event), ["Approval"])
        const tx4 = await marketplace2.buy(productIdBytes32, 100)
        const tr4 = await tx4.wait(0)
        assert.deepStrictEqual(tr4.events.map((e: ethers.Event) => e.event), ["NewSubscription", "Subscribed", undefined])  // 3rd event is token.Transfer
    })
})
